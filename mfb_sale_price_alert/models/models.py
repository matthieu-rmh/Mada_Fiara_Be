...
class SaleOrderLine(models.Model):
    ...
    def _onchange_price_unit_warning(self):
        ...
        elif self.order_id.pricelist_id.id == 2 and self._get_product_price_in_pricelist(self.order_id.pricelist_id.id, self.product_id) != False and self.price_unit < self._get_product_price_in_pricelist(self.order_id.pricelist_id.id, self.product_id) : 
            return {
                'warning': {
                'title': _("Attention pour %s", self.product_id.name),
